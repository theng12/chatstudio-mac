#!/bin/bash
#
# Install Chat Studio KH as an always-on macOS service (launchd LaunchAgent).
#
...
SRV="com.kh.chatstudio.server"
WD="com.kh.chatstudio.watchdog"
PORT=47871
APPNAME="Chat Studio KH"

mkdir -p "$LA" "$ROOT/logs/service" "$ROOT/service"
chmod +x "$ROOT/chatstudio-serve.sh" "$ROOT/chatstudio-watchdog.sh"
...
<dict>
  ...
  <array><string>$ROOT/chatstudio-serve.sh</string></array>
...
<dict>
  ...
  <key>ProgramArguments</key>
  <array><string>$ROOT/chatstudio-watchdog.sh</string></array>
  <key>RunAtLoad</key><true/>
...
echo "   • OpenAI-compatible endpoint at  http://<this-mac>:$PORT/v1"
echo ""
echo "──────────────────────────────────────────────────────────────────────────"
