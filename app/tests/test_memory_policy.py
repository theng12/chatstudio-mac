...
def test_memory_api_frontend_and_process_title(tmp_path, monkeypatch):
    ...
    # this branch made the default follow the host's memory, the label is
    # bound to whatever the backend reports.
...
def test_ui_does_not_hardcode_performance_as_the_default() -> None:
    """The mode picker said "Performance · default". Since the default is
    chosen from the host's memory, a hardcoded label is simply wrong on every
