...
def fit_for(model_min_memory_gb: int) -> dict:
    ...
    The 1.5× threshold is empirical: on Apple Silicon, a loaded LLM + the OS
    itself + a browser usually eats several GB before generation starts, so a
    16 GB Mac running an "≥8 GB" model is actually fine, but a 16 GB Mac
    running a "≥16 GB" model is genuinely tight.
    """
    ...
    if headroom >= 1.0:
        return {
            ...
            "hint": (
                ...
                "Quit Chrome / IDE / other heavy apps before loading the model, otherwise "
                "you may swap to disk and slow generation dramatically."
            ),
            "actual_gb": actual,
            "required_gb": model_min_memory_gb,
        }
