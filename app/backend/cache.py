...
def has_weight_files(repo: str) -> bool:
    ...
    a recognized weight extension. Walks subdirectories because some repos
    keep weights inside component dirs, and huge models shard weights across
    multiple files."""
    snaps = repo_cache_dir(repo) / "snapshots"
    if not snaps.exists():
        return False
...
def list_cached_repos() -> list[str]:
    """Every model repo that is fully cached on disk (has real weight files),
    reconstructed from the HF cache dir names. Includes repos the user
    downloaded via Hub search that aren't in the curated catalog, so they can
    still be loaded and chatted with.

    HF stores repos as `models--{owner}--{name}`; we reverse that. The owner is
    a single token (no `--`), so splitting on `--` and re-joining the remainder
    handles names that themselves contain single hyphens.
    """
    d = hub_dir()
    out: list[str] = []
    if not d.exists():
