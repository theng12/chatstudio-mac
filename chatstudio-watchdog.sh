#!/bin/bash
#
# Chat Studio KH — health watchdog for the always-on server.
#
...
#
PORT=47871
LABEL="com.kh.chatstudio.server"
ROOT="$(cd "$(dirname "$0")" && pwd)"
FAILURE_FILE="${CHATSTUDIO_WATCHDOG_STATE_FILE:-$ROOT/service/.watchdog-failures}"
FAILURES_REQUIRED="${CHATSTUDIO_WATCHDOG_FAILURES_REQUIRED:-3}"
CURL_BIN="${CHATSTUDIO_WATCHDOG_CURL_BIN:-curl}"
LAUNCHCTL_BIN="${CHATSTUDIO_WATCHDOG_LAUNCHCTL_BIN:-launchctl}"
